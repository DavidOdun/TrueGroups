...
class ProfessorPage extends Component {
	...
	apiCallGetQuestions() {
		axios.get("/api/v1/questions/all").then((qRes) => {
	...
	apiCallDeleteQuestions() {
		...
		if (this.state.userDeletedQuestions.length === 0) {
			...
			for (
				var pos = 0;
				pos < this.state.userDeletedQuestions.length;
				pos++
			) {
				axios
					.post(
						"/api/v1/questions/remove/" +
							this.state.userDeletedQuestions[pos]
					)
					.then((addResponse) => {
	...
	apiCallAddQuestions() {
		let filteredQuestion = this.state.userAddedQuestions
			.split("\n")
			.filter((item) => item);
		console.log(filteredQuestion);

		for (var pos = 0; pos < filteredQuestion.length; pos++) {
			if (filteredQuestion[pos] === "") continue;
			let tempForm = {
				question_string: filteredQuestion[pos],
			};
			axios
				.post("/api/v1/questions/add", tempForm)
				.then((addResponse) => {
					console.log("Response Below");
					console.log(addResponse);
				})
	...
	apiCreateClass() {
		...
		if (this.state.classSize > 0 && this.state.className) {
			...
			axios
				.put("/api/v1/classes/create", jForm)
				.then((addResponse) => {
					console.log("Response Below");
					console.log(addResponse);
				})
	...
	structureModalQuestions(formType) {
		...
		for (var pos = 0; pos < this.state.surveyQuestions.length; pos++) {
			if (formType === "/label") {
				formGroupItems.push(
					<Label
						key={pos}
						for={this.state.surveyQuestions[
							pos
						].question_id.toString()}
					>
						{this.state.surveyQuestions[pos].question_id +
							": " +
							this.state.surveyQuestions[pos].question_string}
					</Label>
				);
				...
				selectMultItems.push(
					<option
						key={pos}
						value={this.state.surveyQuestions[pos].question_id}
					>
						{this.state.surveyQuestions[pos].question_string}
					</option>
				);
		...
		if (formType === "/label") {
			userInputItems.push(
				<Form key={0}>
					<FormGroup>{formGroupItems}</FormGroup>
				</Form>
			);
		} else if (formType === "/selectmultiple") {
			userInputItems.push(
	...
	render() {
		if (this.state.pageRedirect) {
			return (
				<Redirect
					to={{
						pathname: this.state.pageRedirect,
		...
		return (
			<div>
				<nav className="navbar navbar-light bg-light justify-content-between">
					...
					{this.state.userInfo.preferred_first_name ? (
						<h1>
							Welcome, {this.state.userInfo.preferred_first_name}
						</h1>
					) : (
					...
					<div>
						...
						<Button
							...
							color="primary"
							onClick={() =>
								this.setState({ pageRedirect: "/editprofile" })
							}
						>
							{" "}
							Edit Profile
						...
						<Button
							size="lg"
							color="secondary"
							onClick={() => this.setState({ pageRedirect: "/" })}
						>
							{" "}
							Logout{" "}
						</Button>
						...
						>
							<div className="card card-body">
								{this.state.surveyQuestions.length === 0 ? (
									<h3>
										No Current Questions. Use the Add button
										below to begin{" "}
									</h3>
								) : (
									<h3>
										{this.structureModalQuestions("/label")}
									</h3>
								)}
							</div>
								...
								<Form>
									<FormGroup>
										...
										<Input
											...
											placeholder="Operating System 90210"
										/>
									</FormGroup>
									<FormGroup>
										<Label for="cSize">
						...
						<div className="modal-content">
							...
							<div className="modal-body">
								<Label for="addQuestion">
									Enter Question(s) Below. SEPERATE MULTIPLE
									QUESTION WITH [ENTER]
								</Label>
								<Input
									type="textarea"
									...
									onChange={(e) =>
										this.setState({
											userAddedQuestions: e.target.value,
										})
									}
							...
							<div className="modal-footer">
								...
								<button
									...
									onClick={() => this.apiCallAddQuestions()}
									data-dismiss="modal"
								>
									Submit Changes
								</button>
