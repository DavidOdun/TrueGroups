...
class StudentPage extends Component {
	...
	apiJoinClass() {
		...
		if (this.state.professorId > 0 && this.state.className) {
			...
			axios
				.post("/api/v1/classes/join", jForm)
				.then((addResponse) => {
					...
					if (addResponse.data.success) {
						...
						this.apiEnrolledClasses();
						this.setState({
							newClassDetail: addResponse.data.class_details,
						});
					}
				})
				.catch((error) => {
	...
	apiSubmitSurvey() {
		...
		if (
			Object.keys(this.state.surveyResponses).length ===
	...
	apiGetClassDetails() {
		if (this.state.userEnrolledClass.length === 0) {
			...
			for (
				var pos = 0;
				pos < this.state.userEnrolledClass.length;
				pos++
			) {
				axios
					.get(
						"/api/v1/classes/details/" +
							this.state.userEnrolledClass[pos].class_code
					)
					.then((addResponse) => {
						console.log("Class Details Response Below");
						console.log(addResponse);
	...
	async structureClassDetails() {
		...
		for (var pos = 0; pos < this.state.userEnrolledClass.length; pos++) {
			...
			console.log(
				"In function User Class Length: " + allUserClass.length
			);
		}
	...
	structureModalQuestions() {
		...
		for (var pos = 0; pos < this.state.surveyQuestions.length; pos++) {
			let qName =
				"Q" + this.state.surveyQuestions[pos].question_id.toString();
			formGroupItems.push(
				<FormGroup key={pos}>
					{/* <Label for={qName}>{qName + ": " + this.state.surveyQuestions[pos].question_string}</Label> */}
					<Label key={pos} for={qName}>
	...
	render() {
		...
		return (
			<div>
				...
				{this.state.completedSurvey ? joinClass : surveyButton}
				<div className="row">
					<div className="col">
						<div
							...
							<div className="card card-body">
								<Form>
									...
									<Button
										color="info"
										size="lg"
										onClick={() => this.apiJoinClass()}
										block
									>
										Submit Join Class Request
									</Button>
								</Form>
							</div>
						</div>
				...
				<div className="row">
					<div className="col">{this.state.classDisplayList}</div>
				</div>
